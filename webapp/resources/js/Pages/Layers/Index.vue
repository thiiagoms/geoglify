...
export default {
    ...
    methods:
    {
        ...
        openDeleteModal(id) {
            this.deleteLayerId = id;
            this.deleteModalOpen = true;
        },

        // Method to close the modal
        closeModal() {
            this.deleteLayerId = null;
        ...
        async deleteUser() {
            ...
            // Delete the user
            await fetch(route('layers.destroy', this.deleteLayerId), {
                method: "DELETE",
            }).then(() => {
                // Reload the items
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <v-breadcrumbs :items="[
                ...
                { title: 'Layers', disabled: true }
            ]" divider="/" />
        </template>

        <v-card class="max-w-7xl mx-auto pa-6" flat  title="Layers" subtitle="List of layers">

            <template v-slot:append>
                <v-btn color="black" variant="tonal" :href="route('layers.create')" class="ml-2">Create</v-btn>
            </template>
                ...
                <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items="serverItems"
                    ...
                    <template v-slot:item.actions="{ item }">
                        <v-btn color="black" class="ml-2" variant="text" density="comfortable" icon
                            :href="route('layers.edit', item.id)" v-if="!item.is_ldap">
        ...
        <v-dialog v-model="deleteModalOpen" width="500">
            <v-card>
                <v-card-title class="pt-4">
                    <span class="headline px-3">Are you sure you want to delete this layers?</span>
                </v-card-title>

                <v-card-text>
                    <v-alert class="mb-4">
                        Once this layers is deleted, all of its resources and data will be permanently deleted.
                    </v-alert>
