      ...
      <v-card-text v-if="!!styleToUpdate">
        <v-form ref="form">
          ...
          <template v-if="layerType === 'point'">
            <v-menu open-on-click :close-on-content-click="false">
              <template v-slot:activator="{ props }">
                <v-text-field v-model="styleToUpdate.lineColor" label="Line Color" placeholder="Enter Line Color" variant="outlined" class="mb-2" readonly v-bind="props">
              ...
              <v-color-picker v-model="styleToUpdate.lineColor"></v-color-picker>
            </v-menu>

            <v-slider v-model="styleToUpdate.lineWidth" label="Line Width" min="1" max="10" step="1" thumb-label class="my-5"></v-slider>

            <v-menu open-on-click :close-on-content-click="false">
              <template v-slot:activator="{ props }">
                <v-text-field v-model="styleToUpdate.fillColor" label="Fill Color" placeholder="Enter Fill Color" variant="outlined" class="mb-2" readonly v-bind="props">
                  <template v-slot:append-inner>
                    <div
                      :style="{
              ...
              <v-color-picker v-model="styleToUpdate.fillColor"></v-color-picker>
            </v-menu>

            <v-slider v-model="styleToUpdate.radius" label="Radius size" min="1" max="20" step="1" thumb-label class="my-5"></v-slider>
          </template>
          ...
          <template v-else-if="layerType === 'line'">
            <v-menu open-on-click :close-on-content-click="false">
              <template v-slot:activator="{ props }">
                <v-text-field v-model="styleToUpdate.lineColor" label="Line Color" placeholder="Enter Line Color" readonly variant="outlined" class="mb-2" v-bind="props">
                  <template v-slot:append-inner>
                    <div
                      :style="{
                        backgroundColor: hexToRgba(styleToUpdate.lineColor),
            ...
            <v-text-field v-model="styleToUpdate.dashArray" label="Dash Array Line" placeholder="Enter Dash Array" variant="outlined" class="mb-2" v-maska:[dashArray] />
          </template>
          ...
          <template v-else-if="layerType === 'polygon'">
            ...
            <v-select v-model="styleToUpdate.fillPattern" :items="patterns" item-text="title" item-value="value" :item-props="itemProps" label="Fill Pattern" placeholder="Select Fill Pattern" variant="outlined" class="mb-2" density="compact">
              <template v-slot:selection="data">
                <v-list-item :title="data.item.title" :prepend-avatar="'./patterns/' + data.item.value + '.png'" class="pa-0 pattern"></v-list-item>
              </template>
              <template v-slot:item="{ props, item }">
                <v-list-item v-bind="props" :title="item.title" :prepend-avatar="'./patterns/' + item.value + '.png'" class="pa-0 pl-2 pattern"></v-list-item>
              </template>
            </v-select>
          </template>
        </v-form>
...
<script>
  ...
  export default {
    props: ["open", "layerType", "style", "layerId"],
    ...
    watch: {
      open(value) {
        if (value) {
          this.styleToUpdate = this.style || configs.getDefaultGeoJSONStyle();
        }
