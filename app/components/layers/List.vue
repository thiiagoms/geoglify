<template>
  <v-navigation-drawer v-model="dialogOpened" :location="$vuetify.display.mobile ? 'bottom' : undefined"
    style="z-index: 1001" permanent :width="$vuetify.display.mobile ? '100%' : '400'" v-if="dialogOpened">
    <v-toolbar class="fixed-bar" color="white" dark style="border-bottom: 1px solid #ccc">
      ...
      <v-btn icon @click="openLayerCreator()" density="compact" :disabled="!isAuthenticated">
    ...
    <v-data-table-server class="layers" density="compact" :items-per-page="itemsPerPage" :headers="headers"
      :items="layers" :items-length="total" :loading="loading" :search="search" item-value="_id"
      ...
      <template v-slot:item.id="{ item }">
        <v-card class="mt-1 mx-1 border" variant="outlined" density="comfortable">
          ...
          <template v-slot:append>
            <v-checkbox-btn v-model="item.is_active" @change="updateLayerFeatures(item)" :disabled="item.loading"
              v-if="!item.loading"></v-checkbox-btn>
  ...
  <LayersEditor :open="openEditDialog" :layerData="layerDataToEdit" :layerId="layerIdToEdit"
  ...
  <LayersStyleEditor :open="openStyleDialog" :style="layerStyleToEdit" :layerType="layerType"
    :layerId="layerIdToEditStyle" @update:open="updateOpenStyleDialogState" />
</template>
